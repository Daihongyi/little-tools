...
def main():
    ...
    main_window.title("little tools 1.7")
    ...
    def handle_encipher():
        ...
        encipher_window.geometry("300x200")
        text_entry = tk.Entry(encipher_window, width=50)
        text_label = tk.Label(encipher_window, text="请输入需要加密的文本")
        key_entry = tk.Entry(encipher_window, width=50, show='*')
        key_label = tk.Label(encipher_window, text="请输入密钥")
        ...
        encrypt_button = tk.Button(encipher_window, text="加密", command=encrypt_action)
        exit_button = tk.Button(encipher_window, text="退出", command=encipher_window.destroy)

        text_entry.place(x=140, y=20, width=100)
        key_entry.place(x=75, y=60, width=100)
    ...
    def handle_decrypt():
        ...
        encipher_text_label = tk.Label(decrypt_window, text="加密文本：")
        ...
        decrypt_button = tk.Button(decrypt_window, text="解密", command=decrypt_action)
        exit_button = tk.Button(decrypt_window, text="退出", command=decrypt_window.destroy)

        encipher_text_entry.place(x=75, y=20, width=150)
        secret_key_entry.place(x=50, y=60, width=150)
    ...
    def handle_new_edition():
        messagebox.showinfo("新版本介绍","版本号:1.7 更新内容:加密与解密使用AES算法，并完成了全系更换GUI")
