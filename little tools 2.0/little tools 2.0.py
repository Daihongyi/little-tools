...
def main():
    ...
    def gui_random_numbers():
        ...
        remake_yes_button = tk.Radiobutton(random_numbers_window, text="允许重复", variable=remake_var, value=1)
        remake_no_button = tk.Radiobutton(random_numbers_window, text="不允许重复", variable=remake_var, value=0)
        remake_yes_button.place(x=10, y=110)
        remake_no_button.place(x=140, y=110)
        ...
        def get_random_numbers():
            try:
                ...
                remake = remake_var.get()  # 获取RadioButton的状态
            except ValueError:
            ...
            if max_val < least_val:
                ...
                return
            random_numbers = tools_randint(least_val, max_val, count, bool(remake))
    ...
    def gui_now_time():
        ...
        if not time_on:
            ...
            update_time()
        if time_on:
            time_label.config(fg=random.choice(color_list))




    def gui_new_edition():
        messagebox.showinfo("新版本介绍", "版本号:2.0 更新内容:移除在线时间API以优化性能\n随机数生成时使用选项框选择允许重复\n性能优化")
